...
$re = "SELECT h.*,u.*,u.id AS userid FROM users u,hired h WHERE h.jobid = $jid AND h.checkedin = 1 AND u.id = h.employerid AND h.employeeid = $me";
$rea = $conn->query($re);
if ($rea->num_rows > 0) {
while($e = $rea->fetch_array()){
  ...
  $uhd = "UPDATE `hired` SET `rated` = '1' WHERE `hired`.`jobid` = $jid AND  `hired`.`employeeid` = $me";
  $uhda = $conn->query($uhd);
...
<?php
$myid = $_SESSION['id'];
$ev = "SELECT * FROM jobs WHERE posted_by = $myid AND completed = 1 ORDER BY post_date DESC";
...
<div class="modal fade" id="ratingmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title font-weight-bold" id="exampleModalLongTitle">HOW WAS THIS EMPLOYER ?</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<?php
$myid = $_SESSION['id'];
$ev = "SELECT h.*,j.*,u.*,j.id as jidd,j.city AS jcity , j.country AS jcountry FROM hired h,jobs j,users u WHERE h.employeeid = $myid AND h.completed = 1 AND j.id = h.jobid AND u.id = h.employerid ORDER BY j.enddate DESC";
...
<div class="col-8 px-1">
<div class="row font-weight-bold" >
<i class="fas fa-hard-hat text-secondary my-auto" aria-hidden="true"></i>&nbsp;<?php echo $jn; ?></div>
<div class="row" ><i class="fa fa-certificate text-secondary my-auto " style="margin-left:2px !important;"aria-hidden="true" ></i>&nbsp;<?php echo $name; ?></div>
...
<?php if($rs == 0 && $ci == 1){?>
    ...
    <a type="button" id="rt<?php echo $jidd; ?>"class="btn btn-sm z-depth-1 font-weight-bold" style="position:absolute;bottom:0px !important;right:130px !important;font-size:10px !important;background-color: <?php echo $tt; ?>;"><i class="fa fa-star-o"></i>&nbsp;&nbsp;  RATE EMPLOYER</a>

 <?php } ?> 


<a type="button" id="checkin<?php echo $jidd; ?>"class="btn btn-sm z-depth-1 font-weight-bold" style="position:absolute;bottom:0px !important;right:0px !important;font-size:10px !important;background-color: <?php echo $tt; ?>;width:120px;"><i class="fas fa-info"></i>&nbsp;&nbsp;  DETAILS</a>
...
<?php }}else{?>
  ...
  <div class="align-items-middle text-center center-text my-auto" style="margin-top: 35% !important;">
        ...
        <i class="far fa-sad-tear mb-4" style="color:<?php echo $tt2; ?> !important;font-size: 80px;"></i>
        <br />
        <span style="">NO COMPLETED JOBS YET<br /></span>
